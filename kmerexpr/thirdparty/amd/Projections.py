...
class SimplexProjectionExpSort(SimplexProjectionExp):
    ...
    def project(self, x, g):
        ...
        d = len(x)
        b = np.max(-g)    #included for numerical stabilty in y/sum(yy)
        y = (x+epsilon)*np.exp(-g-b)    
        yy = sorted(y)
        S = sum(yy)
        i = 0
        while((1+epsilon*(d-i))*yy[i]/S - epsilon <= 0):
            S -= yy[i]
            i += 1
        return np.maximum(0, -epsilon+(1+epsilon*(d-i))*y/S)
# ######################################################################################################################
# Unconstrained
# ######################################################################################################################
