    ...
    for count in range(max_iter):
        ...
        if armijo:
            ...
            q_val = (
                loss
                + grad @ (x_new - x)
                - (1 / a) * x_new[mask] @ np.log(x_new[mask] / x[mask])
            )  # q(x_new,a)
...
def update_records(
    grad, normg0, loss, loss0, x, iter, xs, norm_records, loss_records, iteration_counts
):
    ...
    for iter in range(n_iters):
        if type(lrs) == str:
            ...
            if lrs == "armijo":
                ...
                armijo = False
            x_new, lrst = linesearch(
                x, grad, loss, loss_grad, a_init=lrst, armijo=armijo
            )
        else:
            if lrs is None:
        ...
        if np.isnan(x_new.sum()):
            print(
                "iterates have a NaN a iteration ",
                iter,
                " existing and return previous iterate",
            )
            break
