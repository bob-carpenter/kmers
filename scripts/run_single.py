...
def plot_errors_and_scatter(problem, model_parameters):
    ...
    if model_parameters.model_type == "simplex":
        ...
        plot_error_vs_iterations(
            dict_results, theta_true, title_errors, model_parameters.model_type
        )
    ...
    plot_scatter(
        title + "-theta", theta_opt, dict_simulation["theta_sampled"], horizontal=False
    )
    plot_scatter(
        title + "-theta",
        theta_opt,
        theta_opt - dict_simulation["theta_sampled"],
        horizontal=True,
    )
    ...
    print(
        "L1 distance to theta_true = ",
        str(np.linalg.norm(theta_true - theta_opt, ord=1)),
    )
    print(
        "L1 distance to theta_sampled = ",
        str(np.linalg.norm(dict_simulation["theta_sampled"] - theta_opt, ord=1)),
    )
...
def run_model(problem, model_parameters, n_iters=5000, force_repeat=False):
    ...
    if not os.path.exists(X_FILE) or not os.path.exists(Y_FILE) is force_repeat is True:
        print("Generating ", X_FILE, Y_FILE)
        tr.transcriptome_to_x_y(problem.K, ISO_FILE, X_FILE, Y_FILE, L=problem.L)
...
if __name__ == "__main__":
    ...
    # problem = Problem(
    #     filename="sampled_genome_" + str(0.01), K=15, N=5000000, L=100, alpha=0.1
    # )  # alpha=0.1, 0.5
    ...
    tic = time.perf_counter()
    sr.simulate_reads(
        problem, force_repeat=force_repeat
