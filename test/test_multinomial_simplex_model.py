...
def test_optimizer():
    ...
    dict_sol_softmax = model_softmax.fit(theta0_softmax, factr=0.0001, gtol=1e-17, n_iters=500)
    np.testing.assert_allclose(model_softmax.logp_grad(dict_sol['x'])[0], model_softmax.logp_grad(dict_sol_softmax['x'])[0], rtol=1e-3, atol=0.3)   
    os.remove(X_FILE)
