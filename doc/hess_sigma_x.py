...
axes.plot_surface(
    ...
    x2,
    -np.log(
        xs[0] * np.exp(x1) / (np.exp(x1) + np.exp(x2))
        + xs[1] * np.exp(x2) / (np.exp(x1) + np.exp(x2))
    ),
)
...
l = lambda a: -np.sum(np.log(X @ np.exp(a) / (np.sum(np.exp(a))))) + a @ a / (
    2 * 100 * 100
)
...
l0 = lambda a: -np.log(
    (X @ np.exp(a) + 1) / (np.sum(np.exp(a)) + 1)
)  # With bijective softmax
testnum = 100
...
for x in a1:
    for y in a2:
        ...
        # print(" x, y, l(x), l(y) = ", x, y, l(x), l(y))
        conv_test.append(
            np.maximum(ftest(x), ftest(y)) - ftest(lamb * x + (1 - lamb) * y)
        )
...
Hessli = (
    -Dsigs @ np.diag(xs) / xsTsigs
    + np.outer(Dsigs @ xs, sigs * xs) / (xsTsigs**2)
    + Dsigs
)
...
Hess = (
    np.diag(sigs * xs)
    - np.diag(sigs) * (sigs @ xs)
    - (np.outer(sigs, sigs * xs) + np.outer(sigs * xs, sigs))
    + 2 * np.outer(sigs, sigs) * (sigs @ xs)
)
