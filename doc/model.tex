...
\rob{Whats $G$, and what does the notation $T_{g, 1:K_g} $ mean? I know matlab notation $T_{g: 1:K_g}$ is the  column elements between $1:K_g$ on thje $g$th row of $T$, but I don't know $g,$ $K_g$ or why it appears here.
Also I don't know what multiset means :)}
...
\rob{I don't know what is meant by "a gap between transcipts" }
...
\rob{So  $\textrm{count}_K(X_n) $ is a vector in $\mathbb{B}^K$? }
...
the same result,
\rob{The softmax is  not injective since
 }
\[
  \textrm{softmax}(\alpha)
...
\rob{We can however make the softmax function into a one-to-one mapping by ... extending.... restricting...}
