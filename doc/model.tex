...
\subsection{Projecting on the Simplex}
...
\end{align} 
As for the prior $p(\theta)$, since $\theta$ is in the simplex, we now choose a Dirchlet prior where
\begin{equation}
p(\theta) \; = \; \frac{1}{B(\beta)} \prod_{i=1}^G \theta_i^{\beta_i -1}, \quad \mbox{where}\quad B(\beta) = \frac{\prod_{i=1}^G \Gamma(\beta_i)}{\Gamma(\sum_{i=1}^G \beta_i)}
\end{equation}
