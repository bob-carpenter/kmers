...
\begin{lemma}
%Let $\supp(y) = \{i \; : \; y_i \neq 0 \}$ be the support of $y.$ For a set of vectors $\{v_1, \ldots, v_m \}$ let
% $$\cone(v_1, \ldots, v_m) := \left\{ \sum_{i=1}^m v_i \alpha_i \; : \;  \alpha \geq 0\right\},$$
% be the cone formed by these vectors.
% If
%\begin{equation}\label{eq:zeronocone}
%0 \not\in \cone( x_i \; : \; y_i \neq 0),
%\end{equation}
If $y \neq 0$
...
%Thus~\eqref{eq:temploz494} states that $ \sum_{j \in \supp(y)}\alpha_j x_j =0$, which is equivalent to $0 \in  \cone( x_i \; : \; y_i \neq 0),$ which is not possible due to our assumption~\eqref{eq:zeronocone}.
...
\end{align}
\rob{Why not line search so that the model on the right is an upper bound on $f(\theta^{t+1})$? This would guarantee descent.  Search "exponentiated gradient descent mirror descent with backtracking line search."}
